    ...
    try:
        with get_db_connection() as conn:
            with conn.cursor() as cur:
                ...
                # ✅ REMOVED STATUS-RELATED QUERIES (status column doesn't exist)
    ...
    try:
        ...
        with get_db_connection() as conn:
            with conn.cursor() as cur:
                ...
                # ✅ Get paginated data - UPDATED FOR YOUR TABLE STRUCTURE
                query = f"""
                    SELECT id, profile_pic, full_name, phone, email, location, 
                           created_at, 
                           created_at as last_updated  # ✅ updated_at column doesn't exist
                    FROM users 
                    WHERE {where_clause}
...
def get_user_details(user_id):
    ...
    try:
        with get_db_connection() as conn:
            with conn.cursor() as cur:
                # ✅ UPDATED FOR YOUR TABLE STRUCTURE (no updated_at column)
                cur.execute("""
                    SELECT id, profile_pic, full_name, phone, email, location, 
                           password, created_at  # ✅ updated_at removed
    ...
    try:
        ...
        with get_db_connection() as conn:
            with conn.cursor() as cur:
                ...
                # ✅ updated_at column doesn't exist in your table
...
def update_user_status(user_id):
    """Update user status - DISABLED (status column doesn't exist)"""
    return jsonify({
        'success': False, 
